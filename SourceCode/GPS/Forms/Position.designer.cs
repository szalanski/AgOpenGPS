...
namespace AgOpenGPS
{
    public partial class FormGPS
    {
                ...
                case "Fix":
                    {
                        ...
                        if (!isFirstHeadingSet) //set in steer settings, Stanley
                        {
                            ...
                            if (stepFixPts[2].isSet == 0)
                            {
                                ...
                                if (vehicle.Vehicle.AntennaOffset != 0)
                                {
                                    for (int i = 0; i < 3; i++)
                                    {
                                        stepFixPts[i].easting = (Math.Cos(-gpsHeading) * vehicle.Vehicle.AntennaOffset) + stepFixPts[i].easting;
                                        stepFixPts[i].northing = (Math.Sin(-gpsHeading) * vehicle.Vehicle.AntennaOffset) + stepFixPts[i].northing;
                                    }
                                }
                                ...
                                if (ahrs.imuRoll != 88888)
                                {
                                    ...
                                    rollCorrectionDistance = Math.Tan(glm.toRadians((ahrs.imuRoll))) * -vehicle.Vehicle.AntennaHeight;
                        ...
                        if (vehicle.Vehicle.AntennaOffset != 0)
                        {
                            pn.fix.easting = (Math.Cos(-gpsHeading) * vehicle.Vehicle.AntennaOffset) + pn.fix.easting;
                            pn.fix.northing = (Math.Sin(-gpsHeading) * vehicle.Vehicle.AntennaOffset) + pn.fix.northing;
                        }
                        ...
                        {
                            //change for roll to the right is positive times -1
                            rollCorrectionDistance = Math.Sin(glm.toRadians((ahrs.imuRoll))) * -vehicle.Vehicle.AntennaHeight;
                        ...
                        if (ahrs.imuHeading != 99999)
                        {
                            ...
                            if (isReverse)
                                newGPSHeading -= glm.toRadians(vehicle.Vehicle.AntennaPivot / 1
                ...
                case "VTG":
                    {
                        ...
                        if (vehicle.Vehicle.AntennaOffset != 0)
                        {
                            pn.fix.easting = (Math.Cos(-fixHeading) * vehicle.Vehicle.AntennaOffset) + pn.fix.easting;
                            pn.fix.northing = (Math.Sin(-fixHeading) * vehicle.Vehicle.AntennaOffset) + pn.fix.northing;
                        }
                        #endregion
                ...
                case "Dual":
                    {
                        ...
                        if (ahrs.imuRoll != 88888 && vehicle.Vehicle.AntennaHeight != 0)
                        {
        ...
        private void TheRest()
        {
            ...
            if ( isLogElevation && gridTriggerDistance > 2.9 && patchCounter !=0 && isJobStarted)
            {
                ...
                sbGrid.Append(
                    ...
                    + Math.Round((pn.altitude - vehicle.Vehicle.AntennaHeight),3).ToString(CultureInfo.InvariantCulture) + ","
        ...
        private void CalculatePositionHeading()
        {
            ...
            pivotAxlePos.easting = pn.fix.easting - (Math.Sin(fixHeading) * vehicle.Vehicle.AntennaPivot);
            pivotAxlePos.northing = pn.fix.northing - (Math.Cos(fixHeading) * vehicle.Vehicle.AntennaPivot);
            pivotAxlePos.heading = fixHeading;

            steerAxlePos.easting = pivotAxlePos.easting + (Math.Sin(fixHeading) * vehicle.Vehicle.Wheelbase);
            steerAxlePos.northing = pivotAxlePos.northing + (Math.Cos(fixHeading) * vehicle.Vehicle.Wheelbase);
            ...
            hitchPos.easting = pn.fix.easting + (Math.Sin(fixHeading) * (tool.hitchLength - vehicle.Vehicle.AntennaPivot));
            hitchPos.northing = pn.fix.northing + (Math.Cos(fixHeading) * (tool.hitchLength - vehicle.Vehicle.AntennaPivot));
